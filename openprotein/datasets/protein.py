...
class MoleculeDataset(torch_data.Dataset, core.Configurable):
    ...
    def load_smiles(self, smiles_list, targets, pipeline=None, lazy=False, verbose=0, **kwargs):
        """
        ...
        Parameters:
            ...
            pipeline (Callable, optional): data pipelineation function
...
class ReactionDataset(MoleculeDataset, core.Configurable):
    ...
    @utils.copy_args(data.Molecule.from_molecule)
    def load_smiles(self, smiles_list, targets, pipeline=None, verbose=0, **kwargs):
        """
        Load the dataset from SMILES and targets.
...
class ProteinDataset(MoleculeDataset, core.Configurable):
    ...
    def load_sequence(self, sequences, targets, attributes=None, pipeline=None, lazy=False, verbose=0, **kwargs):
        """
        ...
        Parameters:
            ...
            pipeline (Callable, optional): protein sequence pipelineation function
    ...
    def load_lmdbs(self, lmdb_files, sequence_field="primary", target_fields=None, number_field="num_examples",
                   pipeline=None, lazy=False, verbose=0, **kwargs):
        """
        ...
        self.load_sequence(sequences, targets, attributes=None, pipeline=pipeline,
                           lazy=lazy, verbose=verbose, **kwargs)
        self.num_samples = num_samples

    @utils.copy_args(data.Protein.from_molecule)
    def load_pdbs(self, pdb_files, pipeline=None, lazy=False, verbose=0, **kwargs):
        """
    ...
    @utils.copy_args(data.Protein.from_molecule)
    def load_pickle(self, pkl_file, pipeline=None, lazy=False, verbose=0, **kwargs):
        """
        Load the dataset from a pickle file.
...
class ProteinPairDataset(ProteinDataset, core.Configurable):
    ...
    def load_lmdbs(self, lmdb_files, sequence_field="primary", target_fields=None, number_field="num_examples",
        ...
        self.load_sequence(sequences, targets, pipeline=pipeline, lazy=lazy, verbose=verbose, **kwargs)
        self.num_samples = num_samples
...
class ProteinLigandDataset(ProteinDataset, core.Configurable):
    ...
    @utils.copy_args(data.Protein.from_sequence)
    def load_sequence(self, sequences, smiles, targets, num_samples, attributes=None, pipeline=None,
    ...
    def load_lmdbs(self, lmdb_files, sequence_field="target", smiles_field="drug", target_fields=None,
                   number_field="num_examples", pipeline=None, lazy=False, verbose=0, **kwargs):
        """
        Load the dataset from lmdb files.
        ...
        num_samples = self.load_sequence(sequences, smiles, targets, num_samples, pipeline=pipeline,
                                         lazy=lazy, verbose=verbose, **kwargs)
        self.num_samples = num_samples
...
def ordered_scaffold_split(dataset, lengths, chirality=True):
    ...
    return torch_data.Subset(dataset, train_idx), torch_data.Subset(dataset, valid_idx), torch_data.Subset(dataset,
                                                                                                           test_idx)
