...
class GraphClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 init_cfg=None):
        super(GraphClassifier, self).__init__(init_cfg)
    ...
    def forward_dummy(self, graph):
        """Used for computing network flops.

        See `mmclassificaiton/tools/analysis_tools/get_flops.py`
        """
        return self.extract_feat(graph, stage='pre_logits')

    def extract_feat(self, graph, stage='neck'):
        """Directly extract features from the specified stage.

        Args:
            graph (Tensor): The input images. The shape of it should be
                ``(num_samples, num_channels, *graph_shape)``.
        ...
        x = self.backbone(graph, graph.node_feature.float())
    ...
    def forward_train(self, graph, gt_label, **kwargs):
        """Forward computation during training.

        Args:
            graph (Tensor): of shape (N, C, H, W) encoding input images.
        ...
        if self.augments is not None:
            graph, gt_label = self.augments(graph, gt_label)

        x = self.extract_feat(graph)
    ...
    def simple_test(self, graph, graph_metas=None, **kwargs):
